...
typedef struct gia_bs_pkg pkg_t;
struct gia_bs_pkg {
	gia_pkg_t * base;
...
static bool form_ast(ctx_t * ctx) {
	pla_lex_init(&ctx->lex, &ctx->repo->hst);

	pla_ast_init(&ctx->ast, &ctx->repo->hst);

	ctx->ast.root = pla_pkg_create(NULL);

	ctx->root = create_pkg(ctx->repo->root, NULL, ctx->ast.root);
...
bool gia_bs_build(gia_repo_t * repo, ul_hs_t * first_tus_name, const wchar_t * file_name) {
