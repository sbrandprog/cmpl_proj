...
static void copy_repo(wnd_data_t * data, gia_pkg_t * pkg, pkg_t ** out) {
	*out = create_pkg(pkg->name);

	pkg_t ** sub_pkg_ins = &(*out)->sub_pkg;

	for (gia_pkg_t * sub_pkg = pkg->sub_pkg; sub_pkg != NULL; sub_pkg = sub_pkg->next, sub_pkg_ins = &(*sub_pkg_ins)->next) {
		copy_repo(data, sub_pkg, sub_pkg_ins);
	}

	tus_t ** tus_ins = &(*out)->tus;

	for (gia_tus_t * tus = pkg->tus; tus != NULL; tus = tus->next, tus_ins = &(*tus_ins)->next) {
		*tus_ins = create_tus(tus->name);
	}
}
static void update_repo_view_proc_nl(void * user_data, gia_repo_t * repo) {
	wnd_data_t * data = user_data;
		...
		case WM_NCCREATE:
			if (data == NULL) {
				...
				*data = (wnd_data_t){ .hw = hw, .ctx = cd->ctx, .ctl_data = { .ctl_ptr = data, .get_w_proc = wa_ctl_get_parent_w, .get_h_proc = wa_ctl_get_parent_h } };
				...
				gia_repo_lsnr_init(&data->repo_lsnr, data, update_repo_view_proc_nl, data->ctx->es_ctx);
			}
			break;
		case WM_NCDESTROY:
			if (data != NULL) {
				gia_repo_lsnr_cleanup(&data->repo_lsnr);
