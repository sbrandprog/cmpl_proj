...
typedef struct gia_bs_lpt_pkg pkg_t;
struct gia_bs_lpt_pkg {
	gia_pkg_t * base;
...
typedef struct gia_bs_lpt_lp_ctx {
	ctx_t * base;
	tus_t * tus;
...
static bool lex_parse_tus_core(lp_ctx_t * ctx) {
	gia_tus_t * base_tus = ctx->tus->base;
...
static pkg_t * create_pkg(ctx_t * ctx, gia_pkg_t * base, pkg_t * parent) {
...
static bool find_and_push_tus(ctx_t * ctx, pkg_t * pkg, ul_hs_t * tus_name) {
	...
	gia_tus_t * base = gia_pkg_find_tus(pkg->base, tus_name);
...
bool gia_bs_lpt_form_pec_nl(gia_repo_t * repo, pla_ec_fmtr_t * ec_fmtr, ul_hs_t * first_tus_name, ira_pec_t * out) {
