...
bool ira_pec_init(ira_pec_t * pec, ul_hst_t * hst, ul_ec_fmtr_t * ec_fmtr)
{
    *pec = (ira_pec_t){ .hst = hst, .ec_fmtr = ec_fmtr };
...

    {
        ...
        if (!ira_pec_get_dt_arr(pec, &pec->dt_ints[IraIntU8], ira_dt_qual_const, &pec->dt_spcl.utf8_str))
        {
            return false;
        }
...
static bool get_listed_dt_copy(ira_pec_t * pec, ira_dt_t * pred, ira_dt_t * out)
{
    switch (pred->type)
    {
        case IraDtVec:
            *out = (ira_dt_t){
                .type = pred->type,
                .vec = { .size = pred->vec.size, .body = pred->vec.body, .qual = pred->vec.qual }
            };
            break;
        case IraDtPtr:
            *out = (ira_dt_t){
                .type = pred->type,
                .ptr = { .body = pred->ptr.body, .qual = pred->ptr.qual }
            };
            break;
        case IraDtTpl:
        {
        ...
        case IraDtStct:
            *out = (ira_dt_t){
                .type = pred->type,
                .stct = { .tag = pred->stct.tag, .qual = pred->stct.qual }
            };
            break;
        case IraDtArr:
        {
            *out = (ira_dt_t){
                .type = pred->type,
                .arr = { .body = pred->arr.body, .qual = pred->arr.qual }
            };
        ...
        case IraDtFunc:
        {
            ...
            *out = (ira_dt_t){
                .type = pred->type,
                .func = { .ret = pred->func.ret, .args_size = pred->func.args_size, .args = new_args, .vas = pred->func.vas }
            };
            break;
        }
        case IraDtEnmn:
...
bool ira_pec_get_dt_vec(ira_pec_t * pec, size_t size, ira_dt_t * body, ira_dt_qual_t qual, ira_dt_t ** out)
{
    ira_dt_t pred = {
        .type = IraDtVec,
        .vec = { .size = size, .body = body, .qual = qual }
    };
...
bool ira_pec_get_dt_tpl(ira_pec_t * pec, size_t elems_size, ira_dt_ndt_t * elems, ira_dt_qual_t qual, ira_dt_t ** out)
{
    ira_dt_t pred = {
        .type = IraDtTpl,
        .tpl = { .elems_size = elems_size, .elems = elems, .qual = qual }
    };
...
bool ira_pec_get_dt_func(ira_pec_t * pec, ira_dt_t * ret, size_t args_size, ira_dt_ndt_t * args, ira_dt_func_vas_t * vas, ira_dt_t ** out)
{
    ira_dt_t pred = {
        .type = IraDtFunc,
        .func = { .ret = ret, .args_size = args_size, .args = args, .vas = vas }
    };
