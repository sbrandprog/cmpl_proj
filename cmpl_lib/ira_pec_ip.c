...
static bool set_mmbr_acc_ptr_data(ctx_t * ctx, inst_t * inst, ira_dt_t * opd_dt, ul_hs_t * mmbr)
{
    switch (opd_dt->type)
    {
        ...
        case IraDtStct:
        {
            ...
            if (body == NULL || !set_mmbr_acc_ptr_data_tpl(ctx, inst, body, mmbr))
            {
                return false;
            }
            break;
...
static bool prepare_insts_opds_vars(ctx_t * ctx, inst_t * inst, ira_inst_t * ira_inst, size_t opd_size, size_t opd)
{
    size_t vars_size = ira_inst->opds[opd_size].size;
    ...
    ul_hs_t ** var_name = ira_inst->opds[opd].hss;
    for (var_t **var = vars, **var_end = var + vars_size; var != var_end; ++var, ++var_name)
    {
        if (*var_name == NULL)
        {
            report(ctx, "[%s]: [%zi]th variable name in [%zi] operand is invalid", ira_inst_infos[inst->type].type_str.str, (size_t)(var_name - ira_inst->opds[opd].hss), opd);
            return false;
        }
        ...
        *var = find_var(ctx, *var_name);
        ...
        if (*var == NULL)
        {
            report(ctx, "[%s]: can not find a variable [%s] for [%zi] operand", ira_inst_infos[inst->type].type_str.str, ira_inst->opds[opd].hs->str, opd);
            return false;
        }
    ...
    for (size_t opd = 0; opd < IRA_INST_OPDS_SIZE; ++opd)
    {
        switch (info->opds[opd])
        {
            ...
            case IraInstOpdVarsSize:
                ...
                break;
            case IraInstOpdVars2:
                if (!prepare_insts_opds_vars(ctx, inst, ira_inst, 2, opd))
                {
                    return false;
                }
                break;
...
static bool prepare_load_val(ctx_t * ctx, inst_t * inst, const ira_inst_info_t * info)
{
    ...
    if (ctx->trg == TrgCmpl)
    {
        ...
        {
            report(ctx, "[%s]: value in opd[1] is non compilable", ira_inst_infos[inst->type].type_str.str);
            return false;
        }
    }
...
static void save_stack_var_off(ctx_t * ctx, var_t * var, mc_reg_t reg, size_t off)
{
    ...
    ul_assert(mc_reg_infos[reg].grps.gpr && mc_size_infos[reg_size].bytes + off <= var->cmpl.size);
...
static void load_stack_var_off(ctx_t * ctx, mc_reg_t reg, var_t * var, size_t off)
{
    ...
    if (bb_gpr->var != var || bb_gpr->reg_size != reg_size || bb_gpr->off != off)
    {
...
static mc_reg_t get_gpr_reg_dt(mc_reg_gpr_t gpr, ira_dt_t * dt)
{
    ...
    switch (dt->type)
    {
        ...
        case IraDtPtr:
            ...
            break;
        case IraDtEnmn:
            reg_size = ira_int_infos[dt->enmn.tag->body->int_type].mc_size;
            break;
        default:
            ul_assert_unreachable();
    }
...
static void emit_prologue(ctx_t * ctx)
{
    ...
    pushmove_mc_inst(ctx, &stack_alloc);
...
static void emit_epilogue(ctx_t * ctx)
{
    ...
    pushmove_mc_inst(ctx, &stack_free);
}
...
static ira_int_type_t get_int_type(ira_dt_t * dt)
{
    switch (dt->type)
    {
        ...
        case IraDtPtr:
            return IraIntU64;
        case IraDtEnmn:
            return dt->enmn.tag->body->int_type;
        default:
            ul_assert_unreachable();
    }
}
...
static void div_int_like(ctx_t * ctx, var_t * opd0, var_t * opd1, var_t * div_out, var_t * mod_out)
{
    ira_int_type_t int_type = get_int_type(opd0->qdt.dt);
...
static void compile_load_val_impl(ctx_t * ctx, var_t * var, ira_val_t * val)
{
    switch (val->type)
    {
        ...
        case IraValImmInt:
        {
            mc_reg_t reg = get_gpr_reg_dt(McRegGprAx, val->dt);
...
static void compile_unr_optr(ctx_t * ctx, inst_t * inst)
{
    ...
    switch (optr->type)
    {
        ...
        case IraOptrBltnArithNegEnmn:
        {
            ...
            mc_reg_t reg = get_gpr_reg_dt(McRegGprAx, inst->opd0.var->qdt.dt);
...
static void compile_bin_optr(ctx_t * ctx, inst_t * inst)
{
    ...
    switch (optr->type)
    {
        ...
        case IraOptrBltnDivEnmn:
            div_int_like(ctx, inst->opd2.var, inst->opd3.var, inst->opd0.var, NULL);
            break;
        case IraOptrBltnModInt:
        case IraOptrBltnModEnmn:
            div_int_like(ctx, inst->opd2.var, inst->opd3.var, NULL, inst->opd0.var);
            break;
        ...
        {
            ira_int_type_t int_type = get_int_type(inst->opd0.var->qdt.dt);
        ...
        case IraOptrBltnNeqEnmn:
        {
            ira_int_type_t int_type = get_int_type(inst->opd2.var->qdt.dt);
...
static void compile_cast_to_int(ctx_t * ctx, inst_t * inst, ira_dt_t * from, ira_dt_t * to)
{
    switch (from->type)
    {
        ...
        case IraDtPtr:
            ...
            break;
        case IraDtEnmn:
            compile_int_like_cast(ctx, inst->opd0.var, inst->opd1.var, from->enmn.tag->body->int_type, to->int_type);
            break;
        default:
            ul_assert_unreachable();
    }
...
static void compile_cast_to_enmn(ctx_t * ctx, inst_t * inst, ira_dt_t * from, ira_dt_t * to)
{
    ira_int_type_t to_int_type = to->enmn.tag->body->int_type;

    switch (from->type)
    {
        case IraDtInt:
            compile_int_like_cast(ctx, inst->opd0.var, inst->opd1.var, from->int_type, to_int_type);
            break;
        case IraDtEnmn:
            compile_int_like_cast(ctx, inst->opd0.var, inst->opd1.var, from->enmn.tag->body->int_type, to_int_type);
            break;
        default:
            ul_assert_unreachable();
    }
...
static void compile_cast(ctx_t * ctx, inst_t * inst)
{
    ...
    switch (to->type)
    {
        ...
        case IraDtPtr:
            ...
            break;
        case IraDtEnmn:
            compile_cast_to_enmn(ctx, inst, from, to);
            break;
        default:
            ul_assert_unreachable();
    }
...
static void compile_insts(ctx_t * ctx)
{
    ...
    for (inst_t *inst = ctx->insts, *inst_end = inst + ctx->insts_size; inst != inst_end; ++inst)
    {
        ...
        compile_inst_proc_t * proc = compile_insts_procs[inst->type];
...
static bool check_var_for_val(ctx_t * ctx, inst_t * inst, size_t opd)
{
    ...
    {
        report(ctx, "[%s]: value in opd[%zi] var is NULL", ira_inst_infos[inst->type].type_str.str, opd);
        return false;
    }
...
static bool execute_read_ptr(ctx_t * ctx, inst_t * inst)
{
    ...
    {
        case IraValImmPtr:
            report(ctx, "[%s]: no read of integer pointers in interpreter mode", ira_inst_infos[inst->type].type_str.str);
            return false;
        case IraValLoPtr:
        {
            ...
            switch (lo->type)
            {
                case IraLoVar:
                    ...
                    break;
                default:
                    report(ctx, "[%s]: read of unsupported language object", ira_inst_infos[inst->type].type_str.str);
                    return false;
            }
...
static bool execute_make_dt_vec(ctx_t * ctx, inst_t * inst)
{
    ...
    {
        report(ctx, "[%s]: failed to get size of vector", ira_inst_infos[inst->type].type_str.str);
        return false;
    }
...
static bool execute_make_dt_tpl(ctx_t * ctx, inst_t * inst)
{
...

    {
        ...
        for (var_t **var = inst->opd3.vars, **var_end = var + elems_size; var != var_end; ++var, ++elem, ++id)
        {
            ...
            {
                report(ctx, "[%s]: one of elements var value is NULL", ira_inst_infos[inst->type].type_str.str);
                free(elems);
                return false;
            }
...
static bool execute_make_dt_func(ctx_t * ctx, inst_t * inst)
{
...

    {
        ...
        for (var_t **var = inst->opd3.vars, **var_end = var + args_size; var != var_end; ++var, ++arg, ++id)
        {
            ...
            if (var_ptr->intr.val == NULL)
            {
                report(ctx, "[%s]: one of arguments var value is NULL", ira_inst_infos[inst->type].type_str.str);
                free(args);
                return false;
            }
...
static bool execute_insts(ctx_t * ctx)
{
    for (inst_t *inst = ctx->insts, *inst_end = inst + ctx->insts_size; inst != inst_end;)
    {
        ...
        execute_inst_proc_t * proc = execute_insts_procs[inst->type];
